...
pattern evident in figure 3.1. Figure 3.2 shows that no such pattern appears 
when examining a non-phylogenetic measure of specialization: number of hosts.

```{r}
source("scripts/specialization_gam_models.r")

ps <- as.data.frame(summary(phylo_model)$s.table) 
ms <- as.data.frame(summary(mass_model)$p.table) 

```

|                   |Effective df|?? df        |Chi-squared  |p-value         |
|-------------------|------------|-------------|-------------|----------------|
|Species            |`r ps$edf`  |`r ps$Ref.df`|`r ps$Chi.sq`|$`r ps$"p-value"`$|


|          |Estimate          |Standard Error        |z-value            |p-value             |
|----------|------------------|----------------------|-------------------|--------------------|
|Intercept |`r ms$Estimate[1]`|`r ms$"Std. Error"[1]`|`r ms$"z value"[1]`|$`r ms$"Pr(>|z|)"[1]`$|
|log(mass) |`r ms$Estimate[2]`|`r ms$"Std. Error"[2]`|`r ms$"z value"[2]`|$`r ms$"Pr(>|z|)"[2]`$|
...
![Figure 3.2](../figures/nhost_phylo_plot.svg)
Figure 3.2 Number of hosts a parasite has against the host phylogeny. Each point
represents an interaction between an odonate and mite, as such, multiple points
refer to the same mite.
