...
The engorgment process may take a few days to complete, upon which they detach
...
To account for small amounts of cross-contamination we considered 500 reads or 
more of an ASV to be a detection of an interaction. A probabilistic network was 
also constructed by dividing the number of interactions detected between a mite 
and an Odonate with the total number of hosts recorded for that mite ASV. 

## 2.4	Determination of phylogenetic scale

The phylogenetic scale of each mite’s interactions on the Odonate phylogeny were
calculated as a weighted sum of node ages in the phylogeny. Each node was 
weighted by how often it occurs as the most recent common ancestor of pairs of 
host species. In the case of probabilistic networks, the weight of each 
interaction is also taken into account in the weighting.

For a particular mite, the cartesian product is taken of the set of host species
with itself. The most recent common ancestor is found for each of these pairs of
species (including self-pairs) and the coefficients describing the interaction:
zero or one for a binary network, or a range from 0 to 1 for a probabilistic 
network is added to that node. Each node of the phylogeny now has a number 
proportional to how often its descendant nodes are recorded hosts for a specific
parasite. These are rescaled to a number between 0 and 1 by dividing each but 
the sum across the whole phylogeny. These numbers are used to weight the age of 
each node, calculated as the difference between the depth of a node and the 
maximum depth of any node in the phylogeny. The sum of these weighted ages is 
the phylogenetic scale of parasitism in the host’s phylogeny. This method 
requires an ultrametric phylogeny. The values take on the units of the 
phylogeny, whether it be millions of years or some measure of genetic distance. 
